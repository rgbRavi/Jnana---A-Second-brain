...
import { saveTempFile } from '../core/notes'
...
export function useDocumentUpload({
  ...
  /**
   * Import the document at `path`, asking the user how (convert / extract /
   * link, or table / link for a spreadsheet). Takes a **path**, not a File,
   * because every step downstream does: LibreOffice conversion, pandoc
   * extraction, the spreadsheet reader, and the `external://` chip's URL.
   *
   * `displayName` labels the external-link chip; it defaults to the path's own
   * basename and is passed explicitly for a paste, whose staging file on disk
   * is named generically.
   */
  const importDocumentFromPath = async (selected: string, displayName?: string) => {
    const ext = extensionOf(selected)
    ...
    if (ext === 'pdf') {
      ...
      if (choice === 'pdf') {
        ...
        // Only the extraction itself is Pandoc’s business — the insert that
        // follows stays outside this try, or an unrelated editor error gets
        // reported to the user as a missing Pandoc.
        let text: string
        try {
          text = await extractText(selected)
        } catch (err) {
        ...
        try {
          const originalName = displayName ?? selected.split(/[\\/]/).pop() ?? 'document'
          const filename = await importMedia(selected, noteId)
      ...
      if (choice === 'table' && canTable) {
        ...
        try {
          const originalName = displayName ?? selected.split(/[\\/]/).pop() ?? `spreadsheet.${ext}`
          const filename = await importMedia(selected, noteId)
          registerMediaRef(noteId, 'document', filename).catch((e) => log.error('registerMediaRef failed', e))
          const assetPath = await getAssetPath(filename)
          onInsertMarkdown(`\n[External: ${originalName}](external://${encodeURIComponent(assetPath)})\n`)
  ...
  /** Run `body` with the uploading flag + a catch-all toast around it. */
  const withUploadState = async (body: () => Promise<void>) => {
    try {
      onUploadStart()
      setUploading(true)
      await body()
  ...
  const handleDocumentUpload = () =>
    withUploadState(async () => {
      const selected = await open({
        multiple: false,
        filters: [{ name: 'Document', extensions: DOCUMENT_EXTENSIONS }],
      })
      if (!selected || typeof selected !== 'string') return
      await importDocumentFromPath(selected)
    })

  /**
   * A document pasted from the clipboard. Only bytes are available, so it is
   * staged to a temp file and then handed to the same importer the dialog
   * uses — a pasted DOCX gets the identical convert / extract / link choice.
   */
  const handleDocumentPaste = (file: File) =>
    withUploadState(async () => {
      const bytes = new Uint8Array(await file.arrayBuffer())
      const path = await saveTempFile(bytes, extensionOf(file.name))
      await importDocumentFromPath(path, file.name)
    })

  /** Import a document already on disk — the drag-and-drop entry point. */
  const handleDocumentPath = (path: string) =>
    withUploadState(() => importDocumentFromPath(path))

  return { handleDocumentUpload, handleDocumentPaste, handleDocumentPath, uploading }
}
